...
namespace Kudan.AR
{
	...
	public class KudanTracker : MonoBehaviour
	{
		...
		void Start()
		{
			...
			else
			{
                ...
                /*#if UNITY_EDITOR
                    if (_trackerPlugin.StartInputFromCamera(_playModeWebcamID, DefaultCameraWidth, DefaultCameraHeight))
#else
				    if (_trackerPlugin.StartInputFromImage(GameObject.Find("btn_place").GetComponent<PlaceMarkerlessObject>().texture_source))
#endif*/
                {
                    // Start tracking
                    if (_startOnEnable)
				...
				{
					ScreenDebugger.Instance.AddDebug("[KudanAR] Failed to start camera, is it already in use?");
        ...
        {
            //ScreenDebugger.Instance.AddDebug("get floor");
		...
		void Update()
		{
            //ScreenDebugger.Instance.AddDebugInFrame("/" + _trackerPlugin, 30);
            ...
            if (_trackerPlugin != null)
			{
                ...
                //ScreenDebugger.Instance.AddDebugInFrame("ProcessFrame", 30);
                _currentTrackingMethod.ProcessFrame();
                //doesnt reach here during image input
                ...
                //ScreenDebugger.Instance.AddDebugInFrame("OK-GetTrackingTexture", 30);

                //ScreenDebugger.Instance.AddDebugInFrame("/"+texture.width, 30);
                //ScreenDebugger.Instance.AddDebugInFrame("/"+texture.height, 30);

                //ScreenDebugger.Instance.AddDebugInFrame("/" + _background, 30);
                ...
                if (_background != null && texture != null)
				{
                    ...
                    if (enableThis)
                    {
                        ...
                        /*Texture2D tex = Texture2D.CreateExternalTexture(texture.width, texture.height, TextureFormat.RGBA32, true, true,
                            new System.IntPtr(0));*/
                    }
                    ...
                    //ScreenDebugger.Instance.AddDebugInFrame(tex.GetPixel(0, 0) + "", 60);
                    
                    //seems to be working only after couple restarts on android
                    ScreenDebugger.Instance.DrawTexture(texture);
