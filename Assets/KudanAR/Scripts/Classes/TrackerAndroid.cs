...
//			Debug.LogError( "[KudanAR] OnApplicationFocus called - focusStatus: " + focusStatus );
			...
			if ( focusStatus )
			{
				if ( m_DeviceIndex > -1 && m_Width > 0 && m_Height > 0 )
				{
                    ...
                    /*Texture2D text = GameObject.Find("btn_place").GetComponent<PlaceMarkerlessObject>().texture_source;
                    StartInputFromImage(text);
                    ScreenDebugger.Instance.AddDebug("StartInputFromImage");*/
				}
				
				if ( m_WasTrackingWhenApplicationPaused )
				{
					StartTracking();
                    //StartInputFromImage(new Texture2D(10, 10));
                    //ScreenDebugger.Instance.AddDebug("2-StartInputFromImage");
...
//			Debug.LogError( "[KudanAR] OnApplicationPause called - pauseStatus: " + pauseStatus );
		...
		{
            ScreenDebugger.Instance.AddDebug("StartTracking");
			...
			if ( m_KudanAR_Instance != null )
			{
                ScreenDebugger.Instance.AddDebug("SetTrackingEnabled");
		...
		{
            //ScreenDebugger.Instance.AddDebugInFrame("UpdateTracking", 30);
			UpdateRotation ();
            ...
            if ( _isTrackingRunning )
			{
				if ( m_KudanAR_Instance != null )
				{
					...
					{
						iTextureHandle = m_KudanAR_Instance.Call<int>( "getTextureHandle" );
						if ( m_TextureHandle != iTextureHandle )
						{
                            ...
                            ScreenDebugger.Instance.AddDebug("CreateExternalTexture");
            ...
            /*Texture2D tex = Texture2D.CreateExternalTexture(_finalTexture.width, _finalTexture.height, TextureFormat.RGBA32, true, true, new System.IntPtr(m_TextureHandle));
            tex.Apply();
            ScreenDebugger.Instance.AddDebugInFrame(m_TextureHandle + "", 60);
            /*ScreenDebugger.Instance.AddDebugInFrame(_finalTexture.GetNativeTexturePtr() + "", 60);*///na GetNativeTexturePtr to padá
            /*
            ScreenDebugger.Instance.AddDebugInFrame(tex.width + "", 60);
            ScreenDebugger.Instance.AddDebugInFrame(tex.height + "", 60);
            ScreenDebugger.Instance.AddDebugInFrame(tex.GetPixel(10,10) + "", 60);*/
...
//			GL.InvalidateState();
        ...
        public override void ArbiTrackGetPose(out Vector3 position, out Quaternion orientation)
        {
            ...
            //ScreenDebugger.Instance.AddDebugInFrame("m_KudanAR_Instance " + m_KudanAR_Instance, 30);

            if (m_KudanAR_Instance != null)
            {

                //ScreenDebugger.Instance.AddDebugInFrame("Call updateArbi" + position, 30);
                m_KudanAR_Instance.Call ("updateArbi", _floorHeight);
                //TODO: compilation does not reach this point
                //ScreenDebugger.Instance.AddDebugInFrame("+++ Call updateArbi" + position, 30);
                ...
                //ScreenDebugger.Instance.AddDebugInFrame("position " + position, 30);
                //ScreenDebugger.Instance.AddDebugInFrame("orientation " + orientation, 30);
